...
\gabcsnippet{(c4)Dó(f)mi(gh)ni(h) est(h) ter(h)ra,(h) et(h) ple(h)ni(h)t<ul>ú</ul>(ixi)do(h) <ul>e</ul>-(g)ius:(h) <v>$\star$</v>(:) or(h)bis(h) ter(h)rá(h)rum,(h) et(h) u(h)ni(h)vér(h)si(h) qui(h) há(h)bi(h)t<v>\uuline{a}</v>nt(g) in(f) <ul>e</ul>(gh)o.(g) (::) <i>Flex:</i> C{o}r(h)de.(g) †(::)}
